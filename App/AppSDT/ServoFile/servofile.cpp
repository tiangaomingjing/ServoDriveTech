...
void ServoFile::updatePrmTreeItem(QTreeWidgetItem *downloadItem, QTreeWidgetItem *dspItem)
{
    ...
    if (m_barCount % 10 == 0) {
        qApp->processEvents();
        emit sendProgressbarMsg(m_barCount % 100, tr("Updating parameters"));
    }
    int sameIndex = -1;
...
bool ServoFile::downloadItem(SevDevice *dev, int axisIndex, QTreeWidgetItem *item)
{
    ...
    if (m_barCount % 10 == 0) {
        qApp->processEvents();
        emit sendProgressbarMsg(m_barCount % 100, tr("Downloading axis%1").arg(QString::number(axisIndex + 1)) + item->text(GT::COL_FLASH_ALLAXIS_NAME));
    }
...
bool ServoFile::uploadItem(SevDevice *dev, int axisIndex, QTreeWidgetItem *item)
{
    m_barCount++;
    if (m_barCount % 10 == 0) {
        emit sendProgressbarMsg(m_barCount % 100, tr("Uploading axis%1").arg(QString::number(axisIndex + 1)) + item->text(GT::COL_FLASH_ALLAXIS_NAME));
        qApp->processEvents();
    }
    if (item->text(GT::COL_FLASH_ALLAXIS_ADDR).compare("-1") != 0) {
